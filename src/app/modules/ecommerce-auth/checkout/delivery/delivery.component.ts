...
export class DeliveryComponent implements OnInit {
  ...
  verifyAuthenticatedUser(): void {
    this._ecommerceAuthService._authService.user.pipe(take(1)).subscribe(user => { //this._ecommerceAuthService._authService.user.pipe().subscribe(user => {
      if (user) {
        this.CURRENT_USER_AUTHENTICATED = user;
        this.CURRENT_USER_GUEST = null;
      } else {
        this._ecommerceAuthService._authService.userGuest.pipe(take(1)).subscribe(guestUser => { //this._ecommerceAuthService._authService.userGuest.subscribe(guestUser => {
