...
@Component({
  ...
  styleUrls: ['./cap-grid.component.css']
})
export class CapGridComponent implements OnChanges, OnDestroy  {

  @Input() currentUrl: string = '';
  @Input() capsProducts: any[] = [];
  @Input() locale: string = '';
  ...
  private sortSizes(a: string, b: string): number {
    const sizeOrder = ['One size', 'Talla única'];
  ...
  onSizeSelect(product: any, size: string): void {
    ...
    // Prepare cart data using CartOrchestratorService
    const productData = {
      product: product,
  ...
  onColorSelect(product: any, colorIndex: number, newImage: string): void {
    ...
    const productIndex = this.capsProducts.findIndex(p => 
      (p.uniqueId || p.id || p._id) === productId
    );
    
    if (productIndex !== -1) {
      this.capsProducts[productIndex].imagen = newImage;
      this.capsProducts[productIndex].currentImage = newImage;
      
      // Force change detection by creating a new reference
      this.capsProducts = [...this.capsProducts];
    }
